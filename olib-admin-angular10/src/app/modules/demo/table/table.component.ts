...
import { TableCol } from '../../common/components/table/table-col.model';
import { TableOption, TableSortOrder, TableStyleClass } from '../../common/components/table/table-option.model';
import { TableAbstract } from '../../common/components/table/table.abstract';
...
export class TableComponent extends TableAbstract{
  ...
  initialize() {
    this.breadCrumbConfig.setBreadCrumb("DEMO","Table");
  }

  initializeCols() {
    this.cols = new Array;
    this.cols.push(new TableCol("vin","VIN","20%",true,true,false,"/#/demo/search"));
    this.cols.push(new TableCol("year","YEAR","20%",true,true,true));
    this.cols.push(new TableCol("brand","BRAND","20%",true,true,false,null,(s : string)=>s.toUpperCase()));
    this.cols.push(new TableCol("color","COLOR","20%",true,true,false));
    this.cols.push(new TableCol("price","PRICE","20%",true,true,false));
  }
  ...
  initializeOptions() {
    this.options = new TableOption;
    this.options.isCheckbox=true;
    this.options.isExport = false;
    this.options.isFilter = true;
    this.options.isPaginator = true;
    this.options.isShowTotal = false;
    this.options.isSort = true;
    this.options.rows = 10;
    this.options.rowsPerPage = ["10","20","30"];
    this.options.showTotalLabel = "Result";
    this.options.emptymessage = "Not Found";
    this.options.styleClass=TableStyleClass.SIZE_SMALL;
    this.options.sortField = "price";
    this.options.sortOrder = TableSortOrder.ASC;
